...
var CityPicker = /** @class */ (function (_super) {
    ...
    CityPicker.prototype.render = function () {
        ...
        var _b = this.state, provinceCode = _b.provinceCode, cityCode = _b.cityCode, districtCode = _b.districtCode, street = _b.street, db = _b.db;
        return db ? (react_1.default.createElement("div", { className: cx('CityPicker', className) },
            ...
            cityCode &&
                ...
                db.district[provinceCode][cityCode] ? (react_1.default.createElement(components_1.Select, { disabled: disabled, options: db.district[provinceCode][cityCode].map(function (item) { return ({
                    label: db[item],
                    value: item
                }); }), value: districtCode, onChange: this.handleDistrictChange })) : null,
            allowStreet && provinceCode ? (react_1.default.createElement("input", { className: cx('CityPicker-input'), value: street, onChange: this.handleStreetChange, onBlur: this.handleStreetEnd, placeholder: __('请输入街道信息') })) : null)) : (react_1.default.createElement(components_1.Spinner, { show: true, size: "sm" }));
    };
    var _a, _b, _c, _d, _e;
